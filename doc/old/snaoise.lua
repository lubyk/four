...
void main( void )
{
  ...
  // vec3 uvw = v_texCoord3D + 0.1*vec3(snoise(v_texCoord3D + vec3(0.0, 0.0, time)),
   ...
   vec3 n = vec3(
    snoise(v_texCoord3D*0.3 - vec3(0.0, 0.0, time * 5.6)),
    snoise(v_texCoord3D *0.3- vec3(0.0, 0.0, time * 4.8)),
    snoise(v_texCoord3D *0.3- vec3(0.0, 0.0, time * 4.9))
    );
...
timer = lk.Timer(step * 1000, function()
  ...
  dynamicsWorld:stepSimulation(step, 5)
  app.win:update()
end)
timer:start()
