...
--]]--
  ...
  geometry = Effect.Shader [[
    ...
    { 
      ...
      vec3 n405 = cross(v4 - v2, v3 - v2);

      if (dot(n042, n021) < 0) n021 = -n021;
      if (dot(n042, n243) < 0) n243 = -n243;
      if (dot(n042, n405) < 0) n405 = -n405;

      if (n042.z * n021.z < 0) 
      {
         gl_Position = cam_to_clip * vec4(v0, 1);
         EmitVertex();
         gl_Position = cam_to_clip * vec4(v2, 1);
         EmitVertex();
         EndPrimitive();
      }
      ...
      if (n042.z * n243.z < 0) 
      {
         gl_Position = cam_to_clip * vec4(v2, 1);
         EmitVertex();
         gl_Position = cam_to_clip * vec4(v4, 1);
         EmitVertex();
         EndPrimitive();
      }
...
local nextGeometry = Demo.geometryCycler { normals = true, adjacency = true }
